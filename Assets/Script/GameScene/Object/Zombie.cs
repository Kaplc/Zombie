...
public class Zombie : MonoBehaviour
{
    ...
    public void AtkEvent()
    {
        ...
        // 根据
        if (target.gameObject == GameManger.Instance.player)
        {
            Collider[] colliders =
                Physics.OverlapSphere(transform.position + transform.forward + transform.up, 2, 1 << LayerMask.NameToLayer("Player"));
            // 不为0必定攻击到目标
        ...
        {
            Collider[] colliders =
                Physics.OverlapSphere(transform.position + transform.forward + transform.up, 2, 1 << LayerMask.NameToLayer("Core"));
            if (colliders.Length != 0)
            {
